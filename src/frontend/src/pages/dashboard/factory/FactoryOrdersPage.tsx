...
export function FactoryOrdersPage() {
  ...
  return (
    <div>
      <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 28, marginBottom: 24 }}>Заказы завода</h1>
          ...
          <div className="panel">
            {orders.map((o: Order) => (
              <Link key={o.id} to={`/dashboard/orders/${o.id}`}
                style={{ display: 'grid', gridTemplateColumns: '1fr 160px 120px 110px 80px', gap: 16, padding: '14px 20px', alignItems: 'center', borderBottom: '1px solid var(--line)', textDecoration: 'none', color: 'inherit', transition: 'background .15s' }}
                className="dash-rrow">
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>{o.product || 'Заказ'}</div>
                  <div style={{ fontSize: 12, color: 'var(--faint)', marginTop: 2 }}>{o.id}</div>
                </div>
                <div style={{ fontSize: 13 }}>{o.consultant?.user?.name || '—'}</div>
